...
export default function DashboardPage() {
    const [activeChat, setActiveChat] = useState<ActiveChat>({ id: null, type: null });

    const contacts = [
        { id: "alex-rivera", name: "Alex Rivera", note: "See you there!" },
        { id: "sarah-chen", name: "Sarah Chen", note: "The project is ready..." },
        { id: "marcus-volt", name: "Marcus Volt", note: "Sent a file." },
    ];

    const isDirectSelected = activeChat.type === "direct";
    const isRoomSelected = activeChat.type === "room";

    function selectContact(id: string) {
        setActiveChat({ id, type: "direct" });
    }
                ...
                <div className="grid h-[calc(100vh-74px)] grid-cols-[270px_1fr]">
                    <aside className="border-r-[3px] border-black px-4 py-4">
                        ...
                        <div className="space-y-3">
                            <button
                                type="button"
                                className={`flex w-full items-center gap-3 border-[3px] border-black px-3 py-3 text-[22px] font-black uppercase ${
                                    isDirectSelected ? "bg-black text-white" : "bg-transparent"
                                }`}
                                onClick={() => {
                                    if (!activeChat.id) {
                                        setActiveChat({ id: "recent-direct", type: "direct" });
                                    }
                                }}
                            >
                                <PersonTabIcon />
                            ...
                            <button
                                type="button"
                                className={`flex w-full items-center gap-3 border-2 border-black px-3 py-3 text-[22px] font-black uppercase ${
                                    isRoomSelected ? "bg-black text-white" : "bg-transparent"
                                }`}
                                onClick={() => {
                                    if (!activeChat.id) {
                                        setActiveChat({ id: "recent-room", type: "room" });
                                    }
                                }}
                            >
                                <RoomsTabIcon />
                        ...
                        <div className="mt-6 border-t-2 border-black pt-4">
                            ...
                            <ul className="mt-4 space-y-5">
                                {contacts.map((contact) => (
                                    <li key={contact.id} className="flex cursor-pointer items-center gap-3" onClick={() => selectContact(contact.id)}>
                    ...
                    <section className="flex h-full flex-col px-8 py-6">
                        ...
                        {!activeChat.id ? (
                            ...
                            <div className="mt-8 flex flex-1 items-center justify-center border-4 border-black bg-[#ededed] shadow-[7px_7px_0_0_#000]">
                                <div className="text-center">
                                    <p className="text-[16px] font-black uppercase tracking-[0.2em]">Selected Chat</p>
                                    <h2 className="mt-3 text-[44px] font-black uppercase leading-none">
                                        {activeChat.type === "room" ? "Room Thread" : "Direct Thread"}
                                    </h2>
                                    <p className="mx-auto mt-4 max-w-115 text-[24px] font-bold uppercase leading-[1.2]">
                                        The dashboard design is now merged here. We’ll normalize the backend message flow next.
                                    </p>
                                </div>
                            </div>
                        )}
